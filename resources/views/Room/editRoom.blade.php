...
<body>
    <div class="m-10">
        ...
        <form action="/room/{{ $roomInfo->id }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="mb-10">
                <img src="{{ asset('storage/' . $roomInfo->r_photo) }}" class="w-40 my-5 rounded-lg"
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="roomNumber" id="roomNumber"
                    ...
                    placeholder=" " value="{{ $roomInfo->roomNumber }}" />
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="total" id="total"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " value="{{ $roomInfo->total }}" />
                <label for="total"
                    class="peer-focus:font-medium absolute text-sm text-gray-600 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                    {{ __('room.total') }}
                </label>
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="price" id="price"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " value="{{ $roomInfo->price }}" />
                <label for="price"
                    class="peer-focus:font-medium absolute text-sm text-gray-600 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                    {{ __('room.price') }}
                </label>
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="radio" name="status" id="active" value="Active" @checked($roomInfo->status == 'Active')>
                <label for="active">{{ __('room.active') }}</label>

                <span class="mx-10">
                    <input type="radio" name="status" id="available" value="Available" @checked($roomInfo->status == 'Available')>
                    <label for="available">{{ __('room.avaliable') }}</label>
                </span>

                <input type="radio" name="status" id="lock" value="Lock" @checked($roomInfo->status == 'Lock')>
